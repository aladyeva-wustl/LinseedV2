---
title: "Two stage optimization for deconvolution"
output: 
  html_notebook: 
    code_folding: hide
---
...
```{r}
## Random X
init_proportions_rows <- sample(nrow(simulation_3_noise$V_row), simulation_3_noise$cell_types)
init_proportions_ <- simulation_3_noise$V_row[init_proportions_rows, ]
simulation_3_noise$init_X <- init_proportions_ %*% t(simulation_3_noise$R)
```
...
toPlot <- melt(data.frame(simulation_3_noise$errors_statistics[,c("iteration","is_D","deconv_error","lamdba_error","beta_error","D_h_error","D_w_error","total_error")]),id.vars="iteration",measure.vars = c("deconv_error", "lamdba_error","beta_error","D_h_error","D_w_error","total_error"))
...
```{r}
simulation_3_noise$coef_der_X <- 1e-07
...
```{r, fig.width=10, fig.height=6}
toPlot <- melt(data.frame(tail(simulation_3_noise$errors_statistics[,c("iteration","is_Omega","D_h_error")],1000)) %>% filter(is_Omega==1),id.vars="iteration",measure.vars = c("D_h_error"))
plt <- ggplot(toPlot,aes(x=iteration,y=value,color=variable)) +
  geom_point(size=0.5) +
  geom_line() + theme_minimal()
ggplotly(plt)
