...
```{r}
restored <- simulation_3_no_noise$new_points %*% simulation_3_no_noise$R
p <- simulation_3_no_noise$cell_types

x <- t(simulation_3_no_noise$new_points)
u <- rowMeans(x)
...
Ae <- restored[indice, ]
new_X <- Ae %*% t(simulation_3_no_noise$R)
```
...
```{r}
restored <- simulation_3_noise$new_points %*% simulation_3_noise$R
p <- simulation_3_noise$cell_types

x <- t(simulation_3_noise$new_points)
u <- rowMeans(x)
y <- x / matrix(kronecker(colSums(x * u), rep(1, p)), nrow=p)

#x <- t(simulation_3_noise$new_points)[2:3,]
#c <- sqrt(max(colSums(x^2)))
#y <- rbind(x, rep(c, ncol(x)))
...
Ae <- restored[indice, ]
new_X <- Ae %*% t(simulation_3_noise$R)
```
...
W_ <- t(simulation_3_noise$S) %*% new_Omega %*% diag(D[,1])
...
```{r}
restored <- simulation_3_high_noise$new_points %*% simulation_3_high_noise$R
p <- simulation_3_high_noise$cell_types

x <- t(simulation_3_high_noise$new_points)
u <- rowMeans(x)
y <- x / matrix(kronecker(colSums(x * u), rep(1, p)), nrow=p)
...
Ae <- restored[indice, ]
new_X <- Ae %*% t(simulation_3_high_noise$R)
```
...
W_ <- t(simulation_3_high_noise$S) %*% new_Omega %*% diag(D[,1])
